...
`disable_test_indexing`, antes de modernização/updates. Para uma instalação, a ordem de migração é:
...
Para múltiplas instalações, veja a [ordem em duas fases](operations.md#ordem-real-da-migração).
O comando independente `update` não acrescenta proteção de indexação.

Single-site: lê a opção no contexto da URL HTTPS final do site atual, atualiza apenas
se necessário com `option update blog_public 0` e confirma `blog_public=0` com
`option get blog_public`. Stderr não vazio nessas operações causa falha, mesmo com exit code zero.
Multisite: utiliza todos os IDs/URLs finais do plano HTTPS persistido pela etapa HTTPS,
derivado da correção Multisite, incluindo subdiretórios e subdomínios. Cada leitura/escrita usa
...
No dry-run, estas duas etapas ficam `PLANNED`, sem suas leituras WordPress ou escritas.
As outras etapas e sondagens do pipeline continuam sujeitas às próprias regras de dry-run.
...
em execuções antigas. Postflight geral e proteção absoluta de acesso ficam fora desta etapa.
A retomada também exige as [verificações gerais de consistência](recovery.md); o replay
do plano não autoriza alterações manuais arbitrárias nos arquivos.
